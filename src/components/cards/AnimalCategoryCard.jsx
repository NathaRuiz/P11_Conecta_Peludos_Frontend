...
const AnimalCategoryCard = () => {
  ...
  return (
    <div>
      <h2 className='text-center text-primaryColor font-bold my-4'>Conoce a tu próximo mejor amigo</h2>
    <div className='flex flex-wrap justify-around gap-y-2'>
      <div className="bg-white shadow-2xl flex flex-col items-center justify-center mx-2 lg:w-1/12 rounded-lg py-1">
        <img src={Huella} alt="Huella" className="w-auto h-auto object-cover" />
        <p className="mt-2 text-sm font-medium">Todos</p>
      </div>
      {categories.map(category => (
        <div key={category.id} className="bg-white shadow-2xl flex flex-col items-center justify-center mx-2 lg:w-1/12 rounded-lg py-1">
          <img src={getImageByCategory(category)} alt={category.name} className="w-[90%] h-auto object-cover" />
